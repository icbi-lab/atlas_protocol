...
def aggregate_duplicate_gene_ids(adata, gene_names):
    ...
    adata_tmp = adata[:, ~(adata.var_names.isin(gene_names))].copy() # not entirely sure that .isin() will work with symbols as substring will be matched, worst case the gene will appear as duplicate in the final adata
